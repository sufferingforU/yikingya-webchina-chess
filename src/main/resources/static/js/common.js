...
window.onload = function () {
    ...
    com.get("playBtn").addEventListener("click", function (e) {
        ...
        com.get("BtnCn").style.display = "none";

    })
    // 开始对弈
    com.get("indexOL1").addEventListener("click", function (e) {
        play.isPlay = true;
        var depth = parseInt(getRadioValue("depth"), 10) || 3;
    ...
    com.sAdd = function () {
        ...
        myajax = $.post({
            url: "add",
            data: {
                firstid: $("#userid").val(), username: $("#username").val()
            },//
            success: function (data) {
                if (data[0] == "ok") {
                    com.get("menuWait").style.display = "none";
                    com.get("menuWe").style.display = "block";
                    com.username = data[1];
                    com.oppo = data[2];
                    //console.log("-data2-"+data[2])
                    document.getElementById("rivalname").innerText = "您的对手是：【" + data[1] + "  】  请尽快开始对局，超时或离开将被判负！";
                    document.getElementById("rivalname1").innerText = "对手：【" + data[1] + "】";
                    document.getElementById("rivalnameM").innerText = "【" + data[9] + "】";
                    document.getElementById("rivalnameR").innerText = "【" + data[10] + "】";
                    document.getElementById("totalnumM").innerText = "总场次：" + data[4];
                    document.getElementById("winnumM").innerText = "获胜场次：" + data[3];
                    document.getElementById("winrateM").innerText = "胜率：" + data[5] + "%";
                    document.getElementById("totalnumR").innerText = "总场次：" + data[7];
                    document.getElementById("winnumR").innerText = "获胜场次：" + data[6];
                    document.getElementById("winrateR").innerText = "胜率：" + data[8] + "%";
                }
            }
        })

    }
    com.get("rivalname1").onmouseover = function () {
        com.get("rivalMsg").style.display = "block";
    }
    com.get("rivalname1").onmouseout = function () {
        com.get("rivalMsg").style.display = "none";
    }
    com.get("mychess").onmouseover = function () {
        com.get("myselfMsg").style.display = "block";
    }
    com.get("mychess").onmouseout = function () {
        com.get("myselfMsg").style.display = "none";
    }
    com.get("webBtn").addEventListener("click", function (e) {
        play.isPlay = true;
        // var depth = parseInt(getRadioValue("depth"), 10) || 3;
        ...
        com.get("chessBox").style.display = "block";
        com.get("bnBox").style.display = "none";
        com.get("showRival").style.display = "block";
        com.get("menuBox").style.display = "none";
    })
    com.get("indexOL1").addEventListener("click", function (e) {
        play.isPlay = true;
    ...
    com.get("clasliBtn").addEventListener("click", function (e) {
        ...
        com.get("menuBox").style.display = "none";
        com.get("bnBox").style.display = "none";
        com.get("bnBox1").style.display = "none";
    })
    ...
    com.get("restartBtn1").addEventListener("click", function (e) {
        if (confirm("是否确定要返回首页？")) {
            document.getElementById("myform07").submit();
        }
    })
    com.get("restartBtn2").addEventListener("click", function (e) {
        if (confirm("是否确定要返回首页？")) {
            document.getElementById("myform07").submit();
        }
    })
    com.get("restartBtn3").addEventListener("click", function (e) {
        if (confirm("是否确定要认输？")) {
            myajax1 = $.post({
                url: "rensu",
                data: {
    ...
    com.get("btnUs1").addEventListener("click", function (e) {
        document.getElementById("myform02").submit();

    })
    com.get("btnUs2").addEventListener("click", function (e) {
        document.getElementById("myform03").submit();
...
com.createMans2 = function (map) {
    for (var i = 0; i < map.length; i++) {
        for (var n = 0; n < map[i].length; n++) {
            var key = map[i][n];
            if (key) {
                com.mans[key] = new com.class.Man2(key);
                com.mans[key].x = n;
                com.mans[key].y = i;
                com.childList.push(com.mans[key])
            }
...
com.bylaw2 = {}
// 车
com.bylaw2.c = function (x, y, map, my) {
    var d = [];
    // 左侧检索
    for (var i = x - 1; i >= 0; i--) {
        if (map[y][i]) {
...
// 马
com.bylaw2.m = function (x, y, map, my) {
    var d = [];
    // 1点
    if (y - 2 >= 0 && x + 1 <= 8 && !play.map[y - 1][x] && (!com.mans[map[y - 2][x + 1]] || com.mans[map[y - 2][x + 1]].my != my)) d.push([x + 1, y - 2]);
    // 2点
...
// 相
com.bylaw2.x = function (x, y, map, my) {
    var d = [];
    if (my === 1) { // 红方
        // 4点半
        if (y + 2 <= 9 && x + 2 <= 8 && !play.map[y + 1][x + 1] && (!com.mans[map[y + 2][x + 2]] || com.mans[map[y + 2][x + 2]].my != my)) d.push([x + 2, y + 2]);
...
// 士
com.bylaw2.s = function (x, y, map, my) {
    var d = [];
    if (my === 1) { // 红方
        // 4点半
        if (y + 1 <= 9 && x + 1 <= 5 && (!com.mans[map[y + 1][x + 1]] || com.mans[map[y + 1][x + 1]].my != my)) d.push([x + 1, y + 1]);
...
// 将
com.bylaw2.j = function (x, y, map, my) {
    var d = [];
    var isNull = (function (y1, y2) {
        var y1 = com.mans["j0"].y;
        var x1 = com.mans["J0"].x;
...
// 炮
com.bylaw2.p = function (x, y, map, my) {
    var d = [];
    // 左侧检索
    var n = 0;
    for (var i = x - 1; i >= 0; i--) {
...
/////////////////////////////////////////////////
com.bylaw2.z = function (x, y, map, my) {
    var d = [];
...
com.class.Man2 = function (key, x, y) {
    this.pater = key.slice(0, 1);
    var o = com.args[this.pater]
    this.x = x || 0;
    this.y = y || 0;
    ...
    this.bl = function (map) {
        var map = map || play.map
        return com.bylaw2[o.bl](this.x, this.y, map, this.my)
    }
}
