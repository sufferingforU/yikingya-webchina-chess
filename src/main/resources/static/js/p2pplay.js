...
play.P2Play = function () {
    //return
    play.my = -1;
    com.canvas.addEventListener("click", play.clickCanvasP2)

}
play.P1Play = function () {
    //return
    play.my = 1;
    com.canvas.addEventListener("click", play.clickCanvasP1)
...
play.clickCanvasP2 = function (e) {
    ...
    if (key) {
        play.P2clickMan(key, x, y);
    } else {
        play.P2clickPoint(x, y);
    }
    play.isFoul = play.checkFoul(); //检测是不是长将
}
//点击棋子，两种情况，选中或者吃子
...
play.P2clickMan = function (key, x, y) {
    if (play.my == -1) {
        var man = com.mans[key];
        //吃子
        if (play.nowManKey && play.nowManKey != key && man.my != com.mans[play.nowManKey].my) {
...
play.P2clickPoint = function (x, y) {
    if (play.my == -1) {
        var key = play.nowManKey;
        var man = com.mans[key];
        if (play.nowManKey) {
